    ...
    getEffectivePermissions: async function(user, logger) {
      ...
      try {
        ...
        user.groups.forEach(function(group) {
          group.group.permissions.forEach(function(groupPermission) {
            ...
            permissions.find(function(permission, index) {
              if (
                permission.permission._id.toString() === groupPermission.permission._id.toString()
              ) {
                matchIndex = index;
                return true;
              }
        ...
        user.permissions.forEach(function(userPermission) {
          let matchIndex = -1;
          permissions.find(function(permission, index) {
            if (permission.permission._id.toString() === userPermission.permission._id.toString()) {
              matchIndex = index;
              return true;
            }
          });
